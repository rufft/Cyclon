...
internal sealed class HcDeletionEventPublisher(ITopicEventSender sender) : IDeletionEventPublisher
{
        ...
        string? correlationId = null, CancellationToken ct = default)
    {
        var ev = new DeletionEvent(
            typeof(T).Name, id, originService,
            reason, correlationId ?? Guid.NewGuid().ToString("N"),
